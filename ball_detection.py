...
def shape_selection(event, x, y, flags, param):
    ...
    # check to see if the left mouse button was released
        ...
        if drawing==True:
            copy = image.copy()
            cv.rectangle(copy,ref_point[0],(x,y),(0,255,0),2)
            cv.imshow("image", copy)

# load the image, clone it, and setup the mouse callback function
image = cv.imread('pool_table_edited.png',1)
clone = image.copy()
...
def detect_coordinates(image_address):
     ...
     cv.createTrackbar('uh', 'Trackbar', 0, 255, nothing)
     cv.createTrackbar('us', 'Trackbar', 255, 255, nothing)
     cv.createTrackbar('uv', 'Trackbar', 255, 255, nothing)
     cv.createTrackbar('min_rad', 'Trackbar', 0, 100, nothing)
     cv.createTrackbar('max_rad', 'Trackbar', 0, 100, nothing)
